...
const ROBOT_ID = "robot_02";
const FEEDS = ["third_person", "wrist"] as const;

export default function Robot02Page() {
  ...
  return (
    <div className="mx-auto flex h-full max-w-[1600px] flex-col gap-4 px-8 py-6">
      ...
      <div className="grid flex-1 grid-cols-[minmax(0,440px)_1fr] gap-4 overflow-hidden">
        <div className="flex flex-col gap-3 overflow-hidden pr-1">
          ...
          />
          <VideoFeed
            src={assetUrl(`/videos/${ROBOT_ID}/wrist.mp4`)}
            title="wrist camera"
            aspect="4/3"
            currentTime={scrub.t}
            playing={effectivePlaying}
            onReadyChange={handler("wrist")}
          />
        </div>
